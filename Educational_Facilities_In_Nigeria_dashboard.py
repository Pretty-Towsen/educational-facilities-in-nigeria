...
def filter_data(df, Facility_Type_Display, management, unique_lga):
...
def display_metrics(filtered_df):
    ...
    with col5:
        Water_Supply_percentage = (filtered_df['Improved_Water_Supply'] == True).sum() / len(filtered_df) * 100 if len(filtered_df) > 0 else 0
        st.metric("💧Improved Water Supply", f"{Water_Supply_percentage:.1f}%")
...
def display_charts(filtered_df):
    ...
    with col1:
       comparison = (filtered_df.groupby('management')[['Total_Students', 'Total_Teachers']].mean().reset_index())   
       fig3= px.bar(
            comparison,
            x='management',
            y=['Total_Students', 'Total_Teachers'],
            barmode='group',
    ...
    filtered_df['Improved_Sanitation'].mean() * 100)

        access = pd.DataFrame({
            'Category':['Water','Sanitation'],
            'Percentage':[Water_Supply_Percentage,Sanitation_Percentage]
